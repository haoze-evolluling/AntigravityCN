...
- **设置面板 (Settings)**：
  ...
  - **账号 (Account)**：Google 账号登录与切换、用量与计费、套餐方案详情、服务条款与隐私政策等。
  - **外观 (Appearance)**：深色 / 浅色 / 跟随系统 / 高对比度主题、对话窗格宽度（紧凑 / 适中 / 宽阔）、字体大小、缩放级别、代码换行与空白符渲染等。
  - **模型与用量 (Models & Usage)**：套餐方案详情 (Plan)、当前方案 (Your Plan)、方案升级提示与按钮 (Upgrade)、模型额外额度 (Model Credits) 与超额扣费 (AI Credit Overages)、Gemini / Claude / GPT 模型每周及 5 小时限额 (Weekly / Five Hour Limit Remaining) 与动态重置倒计时等。
  - **权限与安全 (Permissions & Security)**：权限预设（严格 / 标准 / 极速 / 自定义）、沙箱模式（终端命令隔离、沙箱联网控制）、非工作区文件访问策略、网络与 URL 白名单/黑名单、MCP 工具权限、人工审核模式（始终审核 / 智能体决定）等。
  - **自定义配置 (Customizations)**：全局技能、工作区技能、规则 (Rules)、插件 (Plugins)、钩子 (Hooks)、MCP 服务器管理（添加/配置/刷新/空状态说明）、Token 使用量 (Token Usage)、自定义配置预算百分比与明细折叠/展开等。
...
- **动态模式匹配引擎**：
  - 智能动态翻译模型限额与重置时间（如“您已使用部分每周限额，将在 6 天 21 小时后完全重置。”）。
  - 智能动态翻译 Token 预算与明细（如“98.6% 的自定义配置预算可用。”、“显示 2 项明细”）。
